...
class BIGAN():
    ...
    def build_encoder(self):
        ...
        Input_E_img = Input(shape=self.img_shape)

        temp_x = Flatten()(Input_E_img)
        ...
        temp_x = LeakyReLU(alpha=0.2)(temp_x)
        temp_x = BatchNormalization(momentum=0.8)(temp_x)
        temp_x = Dense(self.latent_dim)(temp_x)

        model = Model(input=Input_E_img, output=temp_x)
        model.summary()
        return model
    ...
    def build_generator(self):
        ...
        temp_x = Dense(512)(Input_g_noise)
        temp_x = LeakyReLU(alpha=0.2)(temp_x)
        temp_x = BatchNormalization(momentum=0.8)(temp_x)
        temp_x = Dense(512)(temp_x)
        temp_x = LeakyReLU(alpha=0.2)(temp_x)
    ...
    def build_discriminator(self):
        ...
        D_inp_concat = concatenate([Latent_var, Flatten()(Input_d_img)])

        temp_x = Dense(1024)(D_inp_concat)
        temp_x = LeakyReLU(alpha=0.2)(temp_x)
        temp_x = Dropout(0.5)(temp_x)
        ...
        model = Model(input=[Latent_var, Input_d_img], output=temp_x)
        model.summary()
        return model
